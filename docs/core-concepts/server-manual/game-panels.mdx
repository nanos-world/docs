...
Configuration and installation of this panel can be quite complex, we recommend you read the instructions fully before proceeding.
...
### Installing the game server {/* #installing-the-game-server */}
...
### Configuring nanos server {/* #configuring-nanos-server */}
...
You can avoid that by removing the parameter from the startup command inside the Pterodactyl egg.

:::

:::info

By default the server runs as the **Playtest** Steam App. To change it, add `--steam_app "game"` to the startup command (see [Server Configuration](/core-concepts/server-manual/server-configuration.mdx#command-line-parameters)).

:::

## AMP Installation {/* #amp-installation */}
...
Congrats! You have installed nanos world server on AMP! You can check how to configure the server below.

:::

### Configuring nanos server {/* #configuring-nanos-server-1 */}
