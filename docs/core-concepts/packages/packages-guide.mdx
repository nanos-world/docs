...
Package folder names are also their unique identifier (used in `Config.toml`, requirements and the Vault), so they must be **lowercase** and contain only letters, numbers and `-` (e.g. `my-awesome-package`), with max 64 characters.

:::

:::tip

Wondering in which order Packages and their `Server/`, `Client/` and `Shared/` scripts are loaded? Check the [Package Loading & Lua Environment](/core-concepts/packages/package-loading-and-lua-environment.mdx) guide.
...
It must have an `index.html` in the root. Please refer to [Loading Screen](/core-concepts/packages/loading-screen.mdx) for more information.
...
Please refer to [C Module](/core-concepts/packages/c-module.mdx) for more information.
...
| **`load_level_entities`** | `script`<br/>`game-mode`<br/>`map` | Whether to load all the static meshes placed in the map level as entities on the client side (accessible through scripting). Only enable it if your Package needs to use them, as it costs performance |
| **`compatibility_version`** | `script`<br/>`game-mode`<br/>`map` | The game version (`major.minor`) at the time this package was created, for granting backwards compatibility between breaking changes. See more [here](/core-concepts/packages/compatibility-versions.mdx) |
...
The values defined are loaded when this Map Package is loaded and can be accessed through the method <MethodReference type="StaticClass" class_name="Server" method_name="GetMapConfig" show_class_name />.
...
## Ignoring Client Folders {/* #ignoring-client-folders */}
