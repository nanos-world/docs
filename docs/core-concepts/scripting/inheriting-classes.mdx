...
Before calling `self.Super:Constructor()`, the entity is not fully spawned yet. Only after calling it you will be able to call other entity methods.
...
```lua showLineNumbers
local children_classes = Prop.GetInheritedClasses()
for _, class in pairs(children_classes) do
    -- 'class' is a custom inherited class! we can spawn it using its constructor
    local p = class(Vector(), Rotator(), "nanos-world::SM_Cube")
end
```
...
### Overriding Existing Methods {/* #overriding-existing-methods */}
...
### Calling Parent Methods {/* #calling-parent-methods */}
...
### Overriding `__newindex` {/* #overriding-__newindex */}
...
### Overriding `__index` {/* #overriding-__index */}
...
function MyNewClass:index(key)
    ...
    -- or you could redirect to another member function instead, e.g.:
    -- return MyNewClass.SetLocation
end
...
### Overriding `__tostring` {/* #overriding-__tostring */}
...
```lua showLineNumbers
MyOtherClass = Prop.Inherit("MyOtherClass")
...
local my_entity = MyNewClass(Vector(), Rotator(), "nanos-world::SM_Cube")
local my_prop = Prop(Vector(), Rotator(), "nanos-world::SM_Cube")
local my_other_entity_inherited_from_prop = MyOtherClass(Vector(), Rotator(), "nanos-world::SM_Cube")
...
#### Multiple Parents Example: {/* #multiple-parents-example */}
...
local my_entity = MyNewSubClass(Vector(), Rotator(), "nanos-world::SM_Cube")

-- Will output:
--  Spawned Prop: MyNewSubClass
--  Spawned MyNewClass: MyNewSubClass
--  Spawned MyNewSubClass: MyNewSubClass
```
...
### Client/Server Synchronization {/* #clientserver-synchronization */}

If you define your entities on both Client and Server side, they will behave properly and in a synchronized way! Example:

:::tip

The easiest way to have the class defined on both sides is to call `Inherit` in a `Shared/` file, and only subscribe to side-specific events in `Server/` or `Client/`.
...
### Custom Remote Events {/* #custom-remote-events */}
...
- <MethodReference type="Class" class_name="Entity" method_name="CallRemoteEvent" />
- <MethodReference type="Class" class_name="Entity" method_name="CallRemotePlayersEvent" />
- <MethodReference type="Class" class_name="Entity" method_name="BroadcastRemoteEvent" />
- <MethodReference type="Class" class_name="Entity" method_name="BroadcastRemoteInRadiusEvent" />
...
function MyNewClass:OnMyCustomRemoteEvent(a, b)
    Console.Log("OnMyCustomRemoteEvent! %s %s %s", tostring(self), a, b)
...
function MyNewClass:OnAnotherRemoteEvent(player, a, b)
    Console.Log("OnAnotherRemoteEvent! %s %s %s %s", tostring(self), tostring(player), a, b)
end
...
local p = MyNewClass(Vector(), Rotator(), "nanos-world::SM_Cube")
p:BroadcastRemoteEvent("MyCustomRemoteEvent", Reliability.Reliable, 123, "abc")
```


### Class Custom Default Values {/* #class-custom-default-values */}
...
### Class Register Event {/* #class-register-event */}
...
Prop.Subscribe("ClassRegister", function(class)
	...
	Console.Log(class.name) -- outputs "My Name"
