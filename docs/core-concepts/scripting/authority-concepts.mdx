...
### Methods and Events Availability {/* #methods-and-events-availability */}
...
#### <AuthorityType.ServerOnly />**`Server Side`** {/* #authoritytypeserveronly-server-side */}

- **Methods:** can be called only in the **Server** side.
- **Events**: will be triggered only in the **Server** side.
- **Classes**: can only be spawned in the **Server** side.

#### <AuthorityType.ClientOnly />**`Client Side`** {/* #authoritytypeclientonly-client-side */}

- **Methods:** can be called only in the **Client** side.
- **Events**: will be triggered only in the **Client** side.
- **Classes**: can only be spawned in the **Client** side. (It won't be synchronized with other players).

#### <AuthorityType.AuthorityOnly />**`Authority Side`** {/* #authoritytypeauthorityonly-authority-side */}
...
#### <AuthorityType.NetworkAuthority />**`Network Authority Side`** {/* #authoritytypenetworkauthority-network-authority-side */}
...
#### <AuthorityType.Both />**`Both Sides`** {/* #authoritytypeboth-both-sides */}

- **Methods:** can be called in both **Server** and **Client** side.
- **Events**: will be triggered in both **Server** and **Client** side.
- **Classes**: can be spawned in both **Server** and **Client** side. If you spawn it on client, it won't be synchronized with other players.

#### <AuthorityType.BothNetAuthorityFirst />**`Both Sides (Network. Authority First)`** {/* #authoritytypebothnetauthorityfirst-both-sides-network-authority-first */}
...
Another important concept in nanos world is the **Network Authority**. We have a _"distributed network authority"_ concept, which means the work of calculating physics are automatically assigned and distributed to the clients.

In general, if a <Classes.Actor /> is near an in-game Character, it's physics will be calculated by that player's computer. The Player that is responsible for calculating and sharing the results is called **Network Authority**.

The Player assigned is automatically calculated by the server and takes some things into consideration:

* The **distance** from the Character to the entity.
* If the Player is **possessing** a <Classes.Pawn />.
...
* If the Character has just **shot** an entity (e.g. <Classes.Prop />, <Classes.Pickable />, <Classes.Vehicle />, etc).
* If the Character has just **rammed** a <Classes.Character /> with a Vehicle.

In all this cases, the Player will be automatically assigned to be the **Network Authority** of that Object. Note that this calculation is only made if the Player is possessing a Character.


### Overriding the current Network Authority {/* #overriding-the-current-network-authority */}

It is possible to override the current Network Authority of a specific entity by using the method <MethodReference type="Class" class_name="Actor" method_name="SetNetworkAuthority" />.

:::tip

Under certain circumstances, actors may not be in **Distributed Network Authority** mode. This means they cannot have their current network authority changed. Example: if a player is driving a Vehicle or using a Weapon, mandatorily they are the Vehicle and Weapon's Network Authority, and this cannot be overwritten through scripting. To validate if an Actor can have it overridden, you can use the method <MethodReference type="Class" class_name="Actor" method_name="IsNetworkDistributed" />.

:::


You can also prevent it from being automatically distributed to other Players by calling <MethodReference type="Class" class_name="Actor" method_name="SetNetworkAuthorityAutoDistributed" params={"false"} />.

:::caution

Always remember to restore the automatic **Network Authority Distribution** by calling <MethodReference type="Class" class_name="Actor" method_name="SetNetworkAuthorityAutoDistributed" params={"true"} />. Otherwise that entity may behave completely weird.

:::


### Checking the Authority in Scripting {/* #checking-the-authority-in-scripting */}

- <MethodReference type="Class" class_name="Entity" method_name="HasAuthority" /> tells if the current side spawned that entity (i.e. it's the **Authority** side).
- <MethodReference type="Class" class_name="Actor" method_name="HasNetworkAuthority" /> tells if the local Player is the current **Network Authority** of that Actor.
- The event <EventReference type="Class" class_name="Actor" event="NetworkAuthorityChange" /> is triggered when the Network Authority of an Actor changes.

For a broader view of what gets synchronized and how, check the [Networking & Replication](/core-concepts/scripting/networking-and-replication.mdx) guide.


### Debugging Network & Network Authority {/* #debugging-network--network-authority */}

There is an option in the settings to **Draw Network Debug** information in the World, you can toggle it in the settings (*Settings -> Debug -> Draw Network Debug*), it will draw squares on each entity and traces representing their networked movement.

![](/img/docs/debug-visualizer.webp)


#### Entity Square ■ Colors Meaning {/* #entity-square--colors-meaning */}
...
#### Entity Arrow → Traces Colors Meaning {/* #entity-arrow--traces-colors-meaning */}
