...
Traces are only available on the **Client** side, as the server doesn't run the physics of the world. If the server needs the result, send it through a [Remote Event](/core-concepts/scripting/events-guide.mdx#remote-events) (and validate it, as the client could send anything).
...
## Other Trace Shapes {/* #other-trace-shapes */}
...
| Shape | Single | Multi |
| :--- | :--- | :--- |
| Line | <MethodReference type="StaticClass" class_name="Trace" method_name="LineSingle" show_class_name /> | <MethodReference type="StaticClass" class_name="Trace" method_name="LineMulti" show_class_name /> |
| Sphere | <MethodReference type="StaticClass" class_name="Trace" method_name="SphereSingle" show_class_name /> | <MethodReference type="StaticClass" class_name="Trace" method_name="SphereMulti" show_class_name /> |
| Box | <MethodReference type="StaticClass" class_name="Trace" method_name="BoxSingle" show_class_name /> | <MethodReference type="StaticClass" class_name="Trace" method_name="BoxMulti" show_class_name /> |
| Capsule | <MethodReference type="StaticClass" class_name="Trace" method_name="CapsuleSingle" show_class_name /> | <MethodReference type="StaticClass" class_name="Trace" method_name="CapsuleMulti" show_class_name /> |

:::tip

The result table only includes some fields (like `Entity` or `SurfaceType`) if you ask for them with the matching <Enums>TraceMode</Enums> flags, such as `TraceMode.ReturnEntity` or `TraceMode.ReturnPhysicalMaterial`.
