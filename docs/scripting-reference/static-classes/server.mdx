...
> Makes a HTTP Request, the result will be returned in the provided Callback function in the format (status, data)
> 
> Note: The Request is made in a Sync way (i.e. the server will freeze during the request, in a future update we should allow async requests)

```lua
Server.HTTPRequest(uri, endpoint, method, content_type, compress, data, headers, callback)
```

| Type | Parameter | Default Value | Description |
| :--- | :--- | :--- | :--- |
| <BasicType.String /> | **`uri`** |  | The main URL |
| <BasicType.String /> | **`endpoint`** | `/` | The endpoint |
| <BasicType.String /> | **`method`** | `GET` |  |
| <BasicType.String /> | **`content_type`** | `application/json` |  |
