...
<ReferenceLink href="/docs/core-concepts/scripting/events-guide">Events Guide</ReferenceLink>
...
| <AuthorityType.ClientOnly /> | [**`CallRemote`**](events#callremote) | Calls an Event on Client which will be triggered in all Server Packages |
| <AuthorityType.ServerOnly /> | [**`CallRemote`**](events#callremote) | Calls an Event on Server which will be triggered in all Client's Packages of `Player` |
...
:::info
On **Server**, registering for remote events has an addition parameter: [Player](/docs/scripting-reference/classes/player), which is the client who sent the event.
:::
