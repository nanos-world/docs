...
| :--- | :--- | :--- | :--- |
| <AuthorityType.Both /> |  | [**`Call`**](events#call) | Calls an Event which will be triggered in all Local Packages |
...
| <AuthorityType.Both /> | <BasicType.Function /> | [**`Subscribe`**](events#subscribe) | Subscribes for an user-created event which will be triggered for both local or _remote\*_ called events |
| <AuthorityType.Both /> |  | [**`Unsubscribe`**](events#unsubscribe) | Unsubscribes from all subscribed events in this Package with that event name, optionally passing the function to unsubscribe only that callback |
