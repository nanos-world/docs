---
title: 👩‍💻 Player
description: Players are Entities that represents the individual behind the mouse and keyboard. Players are spawned automatically when connected to the server.
sidebar_position: 0
tags: [class]
---


:::info

🟥 **Authority:** You cannot spawn or Destroy Players.

:::

Players are Entities that represents the individual behind the mouse and keyboard. Players are spawned automatically when connected to the server.

import { Structs, BasicType, AuthorityType, Classes, Enums } from '@site/docs/components/_nanos.mdx';


## Usage

```lua
-- Spawns and possess a Character when a Player joins the server
Player.Subscribe("Spawn", function(player)
    local new_char = Character()
    player:Possess(new_char)
end)

-- Destroys the Character when the Player leaves the server
Player.Subscribe("Destroy", function(player)
		local character = player:GetControlledCharacter()
		if (character) then
				character:Destroy()
		end
end)
```

## Functions

|  | **Returns** | **Name** | **Description** |
| :--- | :--- | :--- | :--- |
| <AuthorityType.ServerOnly /> |  | [**`Ban`**](player#ban) | Bans the player from the server |
| <AuthorityType.ServerOnly /> |  | [**`Connect`**](player#connect) | Redirects the player to another server |
| <AuthorityType.ServerOnly /> |  | [**`Kick`**](player#kick) | Kicks the player from the server |
| <AuthorityType.ServerOnly /> |  | [**`Possess`**](player#possess) | Bind the `Player` to a Character |
| <AuthorityType.Both /> |  | [**`SetCameraLocation`**](player#setcameralocation) | Sets the Player’s Camera Location |
| <AuthorityType.Both /> |  | [**`SetCameraRotation`**](player#setcamerarotation) | Sets the Player’s Camera Rotation |
| <AuthorityType.Both /> |  | [**`SetCameraSocketOffset`**](player#setcamerasocketoffset) | Sets the Player’s Camera Socket Offset |
| <AuthorityType.Both /> |  | [**`SetCameraArmLength`**](player#setcameraarmlength) | Sets the Player’s Camera Arm Length |
| <AuthorityType.Both /> |  | [**`AttachCameraTo`**](player#attachcamerato) | Attaches the Player’s Camera to an Actor |
| <AuthorityType.Both /> |  | [**`ResetCamera`**](player#resetcamera) | Resets the Player’s Camera to the default state |
| <AuthorityType.Both /> |  | [**`Spectate`**](player#spectate) | Spectates another Player |
| <AuthorityType.ServerOnly /> |  | [**`SetName`**](player#setname) | Sets the player’s name |
| <AuthorityType.Both /> |  | [**`SetValue`**](player#setvalue) | Sets a value in this entity, which can be accessed by any package |
| <AuthorityType.ServerOnly /> |  | [**`SetVOIPChannel`**](player#setvoipchannel) | Sets the VOIP Channel of this Player |
| <AuthorityType.Both /> |  | [**`SetVOIPSetting`**](player#setvoipsetting) | Sets the VOIP Settings of this Player |
| <AuthorityType.Both /> |  | [**`SetVOIPVolume`**](player#setvoipvolume) | Sets the VOIP Volume of this Player |
| <AuthorityType.ServerOnly /> |  | [**`UnPossess`**](player#unpossess) | Release the `Player` from the Character |
| <AuthorityType.Both /> | <BasicType.String /> | [**`GetSteamID`**](player#getsteamid) | Gets the Steam account ID |
| <AuthorityType.Both /> | <BasicType.String /> | [**`GetAccountID`**](player#getaccountid) | Gets the nanos account ID |
| <AuthorityType.Both /> | <BasicType.String /> | [**`GetAccountName`**](player#getaccountname) | Gets the nanos account name |
| <AuthorityType.ClientOnly /> | <Structs.Vector /> | [**`GetCameraLocation`**](player#getcameralocation) | Gets the Player’s Camera Location |
| <AuthorityType.ClientOnly /> | <Structs.Rotator /> | [**`GetCameraRotation`**](player#getcamerarotation) | Gets the Player’s Camera Rotation |
| <AuthorityType.Both /> | <Classes.Character /> | [**`GetControlledCharacter`**](player#getcontrolledcharacter) | Returns the character of the player |
| <AuthorityType.Both /> | <BasicType.Number /> | [**`GetID`**](player#getid) | Gets the network ID of this entity |
| <AuthorityType.ServerOnly /> | <BasicType.String /> | [**`GetIP`**](player#getip) | Gets the IP of this Player |
| <AuthorityType.Both /> | <BasicType.String /> | [**`GetName`**](player#getname) | Returns the player’s name |
| <AuthorityType.Both /> | <BasicType.Number /> | [**`GetPing`**](player#getping) | Returns the ping of a player |
| <AuthorityType.Both /> | <BasicType.String /> | [**`GetType`**](player#gettype) | Returns the type of this Actor |
| <AuthorityType.Both /> | <BasicType.Number /> | [**`GetVOIPChannel`**](player#getvoipchannel) | Returns the VOIP Channel of this Player |
| <AuthorityType.Both /> | <BasicType.Any /> | [**`GetValue`**](player#getvalue) | Gets a value given a key |
| <AuthorityType.ServerOnly /> | <BasicType.Boolean /> | [**`IsHost`**](player#ishost) | Returns if this Player started the server from New Game |
| <AuthorityType.ClientOnly /> | <BasicType.Boolean /> | [**`IsLocalPlayer`**](player#islocalplayer) | Returns if this is the LocalPlayer |
| <AuthorityType.Both /> | <BasicType.Boolean /> | [**`IsValid`**](player#isvalid) | Returns if this entity is valid \(i.e. wasn’t destroyed and points to a valid entity\) |
| <AuthorityType.Both /> | <Enums>VOIPSetting</Enums> | [**`GetVOIPSetting`**](player#getvoipsetting) | Returns this Player VOIP Setting |
| <AuthorityType.Both /> | <BasicType.Function /> | [**`Subscribe`**](player#subscribe) | Subscribes for an Event |
| <AuthorityType.Both /> |  | [**`Unsubscribe`**](player#unsubscribe) | Unsubscribes from all subscribed Events in this Entity and in this Package |


## Events

|  | **Name** | **Description** |
| :--- | :--- | :--- |
|  | [**`Destroy`**](player#destroy) | Triggered when Player disconnects from the server |
|  | [**`Possess`**](player#possess-1) | A `Player` was bound to a Character |
|  | [**`Spawn`**](player#spawn) | Triggered when Player connects to the server |
| <AuthorityType.ServerOnly /> | [**`Ready`**](player#ready) | Triggered when Player is ready \(the client fully joined, loaded the map and all entities and is ready to play\) |
|  | [**`UnPossess`**](player#unpossess-1) | A Character was released from the `Player` |
|  | [**`VOIP`**](player#voip) | When a Player starts/ends using VOIP |


## Functions Detailed

### <AuthorityType.ServerOnly />`Ban`

> Bans the player from the server

```lua
my_player:Ban(reason)
```

| Type | Parameter | Description |
| :--- | :--- | :--- |
| <BasicType.String /> | **`reason`** |  |


### <AuthorityType.ServerOnly />`Connect`

> Redirects the player to another server

```lua
my_player:Connect(IP, password = "")
```

| Type | Parameter | Default Value | Description |
| :--- | :--- | :--- | :--- |
| <BasicType.String /> | **`IP`** |  | Server IP |
| <BasicType.String /> | **`password`** | `""` | Server password |


### <AuthorityType.ServerOnly />`Kick`

> Kicks the player from the server

```lua
my_player:Kick(reason)
```

| Type | Parameter | Description |
| :--- | :--- | :--- |
| <BasicType.String /> | **`reason`** |  |


### <AuthorityType.ServerOnly />`Possess`

> Makes a `Player` to possess and control a `Character`

```lua
my_player:Possess(new_character, blend_time, exp)
```

| Type | Parameter | Description |
| :--- | :--- | :--- |
| <Classes.Character /> | **`new_character`** |  |
| <BasicType.Number /> | **`blend_time`** | `0` | Time to blend from current camera location to possessed pawn |
| <BasicType.Number /> | **`exp`** | `0` | Exponential used to smooth interp, use `0` for linear movement |


### <AuthorityType.Both />`SetCameraLocation`

> Sets the Player's Camera Location (only works if not possessing any Character)

```lua
my_player:SetCameraLocation(location)
```

| Type | Parameter | Default | Description |
| :--- | :--- | :--- | :--- |
| <Structs.Vector /> | **`location`** |  |  |


### <AuthorityType.Both />`SetCameraRotation`

> Sets the Player's Camera Rotation

```lua
my_player:SetCameraRotation(rotation)
```

| Type | Parameter | Default | Description |
| :--- | :--- | :--- | :--- |
| <Structs.Rotator /> | **`rotation`** |  |  |


### <AuthorityType.Both />`TranslateCameraTo`

> Smoothly moves the Player's Camera Location (only works if not possessing any Character)

```lua
my_player:TranslateCameraTo(location, time, exp)
```

| Type | Parameter | Default | Description |
| :--- | :--- | :--- | :--- |
| <Structs.Vector /> | **`location`** |  |  |
| <BasicType.Number /> | **`time`** |  | Time to interp from current camera location to target location |
| <BasicType.Number /> | **`exp`** | `0` | Exponential used to smooth interp, use `0` for linear movement |


### <AuthorityType.Both />`RotateCameraTo`

> Smoothly moves the Player's Camera Rotation

```lua
my_player:RotateCameraTo(rotation, time, exp)
```

| Type | Parameter | Default | Description |
| :--- | :--- | :--- | :--- |
| <Structs.Rotator /> | **`rotation`** |  |  |
| <BasicType.Number /> | **`time`** |  | Time to interp from current camera rotation to target rotation |
| <BasicType.Number /> | **`exp`** | `0` | Exponential used to smooth interp, use `0` for linear movement |


### <AuthorityType.Both />`SetCameraSocketOffset`

> Sets the Player’s Camera Socket Offset (Spring Arm Offset)

```lua
my_player:SetCameraSocketOffset(socket_offset)
```

| Type | Parameter | Default | Description |
| :--- | :--- | :--- | :--- |
| <Structs.Vector /> | **`socket_offset`** |  |  |


### <AuthorityType.Both />`SetCameraArmLength`

> Sets the Player’s Camera Arm Length (Spring Arm length)

```lua
my_player:SetCameraArmLength(length)
```

| Type | Parameter | Default | Description |
| :--- | :--- | :--- | :--- |
| <BasicType.Number /> | **`length`** |  |  |


### <AuthorityType.Both />`AttachCameraTo`

> Attaches the Player`s Camera to an Actor

```lua
my_player:AttachCameraTo(actor, socket_offset, blend_speed)
```

| Type | Parameter | Default | Description |
| :--- | :--- | :--- | :--- |
| <Classes.Actor /> | **`actor`** |  |  |
| <Structs.Vector /> | **`socket_offset`** |  |  |
| <BasicType.Number /> | **`blend_speed`** |  |  |


### <AuthorityType.Both />`ResetCamera`

> Resets the Camera to default state (Unspectate and Detaches)

```lua
my_player:ResetCamera()
```


### <AuthorityType.Both />`Spectate`

> Spectates other Player

```lua
my_player:Spectate(player, blend_speed)
```

| Type | Parameter | Default | Description |
| :--- | :--- | :--- | :--- |
| <Classes.Player /> | **`player`** |  |  |
| <BasicType.Number /> | **`blend_speed`** |  |  |


### <AuthorityType.ServerOnly />`SetName`

> Sets the player's name

```lua
my_player:SetName(new_name)
```

| Type | Parameter | Description |
| :--- | :--- | :--- |
| <BasicType.String /> | **`new_name`** |  |


### <AuthorityType.Both />`SetValue`

> Sets a value in this entity, which can be accessed by any package \(optionally sync on clients if called from server\)
>
> Please refer to [Entity Values](/core-concepts/scripting/entity-values.mdx) for more information

```lua
my_player:SetValue(key, value, sync_on_clients)
```

| Type | Parameter | Default Value | Description |
| :--- | :--- | :--- | :--- |
| <BasicType.String /> | **`key`** |  | Key |
| <BasicType.Any /> | **`value`** |  | Value |
| <BasicType.Boolean /> | **`sync_on_clients`** | `false` | Server side parameter, if enabled will sync this value through all clients |


### <AuthorityType.ServerOnly />`SetVOIPChannel`

> Sets the VOIP Channel of this Player \(will only communicate with other players in the same channel\)

```lua
my_player:SetVOIPChannel(channel)
```

| Type | Parameter | Description |
| :--- | :--- | :--- |
| <BasicType.Number /> | **`channel`** |  |


### <AuthorityType.Both />`SetVOIPSetting`

> Sets the VOIP Settings of this Player

```lua
my_player:SetVOIPSetting(setting)
```

| Type | Parameter | Description |
| :--- | :--- | :--- |
| <Enums>VOIPSetting</Enums> | **`setting`** |  |


### <AuthorityType.Both />`SetVOIPVolume`

> Sets the VOIP Volume of this Player

```lua
my_player:SetVOIPSetting(volume)
```

| Type | Parameter | Description |
| :--- | :--- | :--- |
| <BasicType.Number /> | **`volume`** |  |


### <AuthorityType.ServerOnly />`UnPossess`

> Release the `Player` from the :ref:`Character`

```lua
my_player:UnPossess()
```


### <AuthorityType.Both />`GetSteamID`

> Gets the Steam account ID
>
> Returns <BasicType.String />

```lua
my_player:GetSteamID()
```


### <AuthorityType.Both />`GetAccountID`

> Gets the nanos account ID
>
> Returns <BasicType.String />

```lua
my_player:GetAccountID()
```


### <AuthorityType.Both />`GetAccountName`

> Gets the nanos account name
>
> Returns <BasicType.String />

```lua
my_player:GetAccountName()
```

### <AuthorityType.ClientOnly />`GetCameraLocation`

> Gets the Player's Camera Location
>
> Returns Vector

```lua
my_player:GetCameraLocation()
```


### <AuthorityType.ClientOnly />`GetCameraRotation`

> Gets the Player's Camera Rotation
>
> Returns Rotator

```lua
my_player:GetCameraRotation()
```


### <AuthorityType.Both />`GetControlledCharacter`

> Returns the character of the player. If the player has not yet been assigned a character, nil is returned
>
> Returns Character or <BasicType.Nil />

```lua
my_player:GetControlledCharacter()
```


### <AuthorityType.Both />`GetID`

> Gets the network ID of this entity (same in both client and server)
>
> Returns <BasicType.Number />

```lua
my_player:GetID()
```

### <AuthorityType.ServerOnly />`GetIP`

> Gets the IP of this Player
>
> Returns <BasicType.String />

```lua
my_player:GetIP()
```


### <AuthorityType.Both />`GetName`

> Returns the player's name
>
> Returns <BasicType.String />

```lua
my_player:GetName()
```


### <AuthorityType.Both />`GetPing`

> Returns the ping of a player
>
> Returns <BasicType.Number />

```lua
my_player:GetPing()
```


### <AuthorityType.Both />`GetType`

> Returns the type of this Actor
>
> Returns <BasicType.String />

```lua
my_player:GetType()
```


### <AuthorityType.Both />`GetVOIPChannel`

> Returns the VOIP Channel of this Player
>
> Returns <Enums>VOIPChannel</Enums>

```lua
my_player:GetVOIPChannel()
```


### <AuthorityType.Both />`GetValue`

> Gets a value given a key. Please refer to [Entity Values](/core-concepts/scripting/entity-values.mdx) for more information
>
> Returns <BasicType.Any />

```lua
my_player:GetValue(key, fallback)
```

| Type | Parameter | Default Value | Description |
| :--- | :--- | :--- | :--- |
| <BasicType.String /> | **`key`** |  | Key |
| <BasicType.Any /> | **`fallback`** |  | Fallback Value if key doesn't exist |


### <AuthorityType.ClientOnly />`IsHost`

>  Returns if this Player started the server from New Game menu
>
> Returns <BasicType.Boolean />

```lua
my_player:IsHost()
```


### <AuthorityType.ClientOnly />`IsLocalPlayer`

> Returns if this is the LocalPlayer
>
> Returns <BasicType.Boolean />

```lua
my_player:IsLocalPlayer()
```


### <AuthorityType.Both />`IsValid`

> Returns if this entity is valid \(i.e. wasn't destroyed and points to a valid entity\)
>
> Returns <BasicType.Boolean />

```lua
my_player:IsValid()
```


### <AuthorityType.Both />`GetVOIPSetting`

> Returns this Player VOIP Setting
>
> Returns <Enums>VOIPSetting</Enums>

```lua
my_player:GetVOIPSetting()
```


### <AuthorityType.Both />`Subscribe`

> Subscribes for an Event
> 
> Returns the <BasicType.Function /> callback itself

```lua
my_player:Subscribe(event_name, callback)
```

| Type | Parameter | Default Value | Description |
| :--- | :--- | :--- | :--- |
| <BasicType.String /> | **`event_name`** |  |  |
| <BasicType.Function /> | **`function`** |  |  |


### <AuthorityType.Both />`Unsubscribe`

> Unsubscribes from all subscribed Events in this Entity and in this Package, optionally passing the function to unsubscribe only that callback

```lua
my_player:Unsubscribe(event_name, callback)
```

| Type | Parameter | Default Value | Description |
| :--- | :--- | :--- | :--- |
| <BasicType.String /> | **`event_name`** |  |  |
| <BasicType.Function /> | **`function`** | `nil` |  |


## Events Detailed

### `Destroy`

> Triggered when Player disconnects from the server

```lua
Player.Subscribe("Destroy", function(self)

end)
```

| Type | Parameter | Description |
| :--- | :--- | :--- |
| [Player](player) | `self` |  |

### `Possess`

> A Player is now controlling a Character

```lua
Player.Subscribe("Possess", function(self, character)

end)
```

| Type | Parameter | Description |
| :--- | :--- | :--- |
| [Player](player) | `self` |  |
| <Classes.Character /> | `character` |  |

### `Spawn`

> Triggered when Player connects to the server

```lua
Player.Subscribe("Spawn", function(self)

end)
```

| Type | Parameter | Description |
| :--- | :--- | :--- |
| [Player](player) | `self` |  |

### <AuthorityType.ServerOnly />`Ready`

> Triggered when Player is ready \(the client fully joined, loaded the map and all entities and is ready to play\)

```lua
Player.Subscribe("Ready", function(self)

end)
```

| Type | Parameter | Description |
| :--- | :--- | :--- |
| [Player](player) | `self` |  |

### `UnPossess`

> A Character was released from the `Player`

```lua
Player.Subscribe("UnPossess", function(self, character)

end)
```

| Type | Parameter | Description |
| :--- | :--- | :--- |
| [Player](player) | `self` |  |
| <Classes.Character /> | `character` |  |

### `VOIP`

> When a Player starts/ends using VOIP

```lua
Player.Subscribe("VOIP", function(self, is_talking)

end)
```

| Type | Parameter | Description |
| :--- | :--- | :--- |
| [Player](player) | `self` |  |
| <BasicType.Boolean /> | `is_talking` |  |

