...
🧙 **Note:** Props smaller than radius 30 units will have **CCD** \(Continuous Collision Detection\) enabled to avoid them falling through the floor or other objects. This will slightly increase the performance cost of them! Also they will have Collision disabled against Characters.

:::

:::info

🧙 **Note:** Props bigger than radius 200 units will have **`Grabbable`** off by default. This can be overridden with `:SetGrabbable()`.
...
> When a Character interacts with this Prop (i.e. try to pick it up) - return false to prevent it
