...
| <BasicType.Number /> | **`index`** | `0` | Index to enable (should be `0`, `1` or `2`.) |
...
> Gets if the actor is being destroyed (you can check if inside other callbacks e.g. `Drop` to see if a Pickable is being dropped because it's going to be destroyed)
>
