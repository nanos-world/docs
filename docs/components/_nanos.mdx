...
export const BaseUtilityClass = (name, path) => (
	<Tippy maxWidth={300} animation={"scale-subtle"} placement={"left"} content={<StructToolTip struct_name={name} description={ APIData.BleedingEdge.UtilityClass[name] ? APIData.BleedingEdge.UtilityClass[name].description : "Utility Class not found. Soon™." } />}>
