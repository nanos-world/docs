---
title: Tool Guns
description: How to create new Tool Guns for the Sandbox game-mode
...
## Creating a custom Tool Gun {/* #creating-a-custom-tool-gun */}
...
The values `name`, `image` and `category` are used by **Spawn Menu** to display it properly on it. The `category` must be one of the categories of the `tools` tab: `tool-guns`, `spawners` or `constrainers`.

The other values `description`, `tutorials`, `tips` and `debug_trace` are used by the **ToolGun** to add custom tutorials or behaviors when using it. Those parameters are optional.
...
## Overriding ToolGun Client Methods {/* #overriding-toolgun-client-methods */}
...
## Tool Gun Single & Double Target {/* #tool-gun-single--double-target */}
...
### ToolGunSingleTarget {/* #toolgunsingletarget */}
...
### ToolGunDoubleTarget {/* #toolgundoubletarget */}
