...
After the Package was created, you need to tell the server to load it when it starts. For that, open the `Config.toml` file which was just generated and look for the `packages = []` section on it. Now you just need to add your package name `"my-awesome-package"` in that list!
