...
## Step 1: Download the Server {/* #step-1-download-the-server */}
...
2. Use SteamCMD to [download nanos world™ Dedicated Server](/core-concepts/server-manual/server-installation.mdx).
3. Download the **nanos world™ Dedicated Server** tool from Steam Client.


## Step 2: Creating a Basic Package {/* #step-2-creating-a-basic-package */}
...
To create a new Package, lets use the handy [CLI](/core-concepts/server-manual/command-line-interface.mdx) tool to speed up the process. Open a terminal in your server folder and run the following command:
...
Next, you need to tell the server to load your Package when it starts. To do that, open the `Config.toml` file located in the server folder and find the `packages = []` setting, under the `[game]` section.
...
The `Config.toml` is created automatically the first time the server starts. If you don't have it yet, start the server once (Step 4) and close it, or skip editing it and start the server with `--packages my-awesome-package` instead.

:::


## Step 3: Adding Script functionalities {/* #step-3-adding-script-functionalities */}
...
We recommend using [VSCode](https://code.visualstudio.com/download) editor, as it contains a [nanos world Extension](https://marketplace.visualstudio.com/items?itemName=go-horse-studios.nanos-world) to help while coding!
...
By default the server runs as the **Playtest** Steam App, which is what players currently use to join. The `--steam_app` parameter accepts `playtest` (default) or `game`, and players can only join servers running the same Steam App they launched.
...
```nanos-console title="NanosWorldServer"
   INFO  nanos world (C) Copyright Go Horse Studios. All Rights Reserved.
   INFO  Starting Server at Port: 7777. Version: 1.9.0. Unreal Version: 5.7.4. Map: 'default-blank-map'.
   INFO  Loading Package 'my-awesome-package' (0.0.1)...
 SCRIPT  Loading some Props =D
   INFO  Package 'my-awesome-package' (0.0.1) loaded.
   INFO  Loading Package 'default-blank-map'...
   INFO  Package 'default-blank-map' (1.0.0) loaded.
SUCCESS  Server started!
SUCCESS  Server successfully connected to Steam! VAC Secure!
```

You can see it loaded your package and outputted the text `Loading some Props =D` on it.


## Step 5: Join your server {/* #step-5-join-your-server */}

Now let's see the results in-game! Open nanos world, go to **Find Servers**, click **Quick Connect** and connect to `127.0.0.1:7777`.
...
## Extra Step: Spawning Characters {/* #extra-step-spawning-characters */}
...
### Spawning a Character for the Player {/* #spawning-a-character-for-the-player */}
...
To apply your changes, run the command in the server console: `package reload my-awesome-package`. This will live reload your Package and apply the changes immediately to all Players connected.
...
### Destroying the Character when the Player leaves the Server {/* #destroying-the-character-when-the-player-leaves-the-server */}

Note that the <Classes.Character /> isn't destroyed automatically when the <Classes.Player /> disconnects. For that, we will need to Subscribe for when the Player leaves the server (<EventReference type="Class" class_name="Entity" event="Destroy" /> event) then manually destroy the Character.

Append this code:

```lua title="Packages/my-awesome-package/Server/Index.lua" showLineNumbers
-- When Player leaves the server, destroy its possessing Character
...
To apply your changes, run the command in the server console: `package reload my-awesome-package`.


## Conclusion {/* #conclusion */}

Congratulations! You have finished your first basic Package and learned a lot!

Next, continue with [Your First Game-Mode](/getting-started/your-first-game-mode.mdx), which builds on this Package to add client scripts, a HUD and communication between server and client.
...
<ReferenceLink href="core-concepts/scripting/debugging-and-logging">Debugging & Logging</ReferenceLink>
