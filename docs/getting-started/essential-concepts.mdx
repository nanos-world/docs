...
Packages can be reloaded while the server is running with the console command `package reload my-package`, or loaded and unloaded through scripting with `Server.LoadPackage()` and `Server.UnloadPackage()`! Also, all packages and the whole Lua VM can be reloaded with the console command `package reload all`.
...
If we want a Package to be loaded, we must define them in our Server's [Config.toml](/core-concepts/server-manual/server-configuration.mdx), under the `[game]` section there is a setting for each package type (`game_mode`, `loading_screen`, `map` and `packages`), on which we must list the Package names we want the server to load!
...
Classes are entities that can be spawned, interacted with, and changed as you wish. An example spawning a <Classes.Light /> and <Classes.StaticMesh /> on the server (so they are automatically synchronized to all players) would be like this:
...
-- GetByIndex returns nil if there is no Prop at that index
if (some_prop) then
...
Code in `Shared/` runs on **both** server and every client. If you spawn an entity there, the server spawns one (synchronized to everyone) **and** each client spawns its own local copy. Spawn networked entities on `Server/` only.
...
if (local_player) then
	...
	-- The Player may not be possessing any Character at the moment
...
To configure a Map, we create a Package of type [map](/core-concepts/packages/packages-guide.mdx#map). This type is like `script`, but its `Package.toml` has a `[map]` section with additional settings, such as `map_asset`, which defines which Map Asset this Package will load:

```toml title="Packages/my-map/Package.toml"
[meta]
    title =                 "My Map"
    author =                "Me"
    version =               "1.0.0"
...
[map]
    # the Map Asset to load, in the format [ASSET_PACK]::[ASSET_KEY]
    map_asset =             "my-asset-pack::MyLevel"
    # asset packs this map needs
    assets_requirements =   [ "my-asset-pack" ]
...
We can configure which map the server will run in the `Config.toml`, with the `map = "my-map"` setting under the `[game]` section.
