...
In nanos world, our API is based on **Events**, which are similar to Hooks in other games. Everything in the API comes through events, such as when a <Classes.Player /> spawns (<EventReference type="Class" class_name="Entity" event="Spawn" is_base />), a <Classes.Character /> enters a vehicle (<EventReference type="Class" class_name="Character" event="EnterVehicle" />) or takes damage (<EventReference type="Class" class_name="Damageable" event="TakeDamage" is_base />). You can listen to these events using Lua scripting and perform actions accordingly.
