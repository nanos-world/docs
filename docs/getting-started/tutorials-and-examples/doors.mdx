...
-- Counts how many Characters are inside the trigger, so the door only closes when everyone left
local characters_inside = 0

-- Registers the trigger when a Character moves in
trigger:Subscribe("BeginOverlap", function(trigger, actor)
    -- IsA also matches classes inherited from Character
    if (not actor:IsA(Character)) then return end

    characters_inside = characters_inside + 1
...
trigger:Subscribe("EndOverlap", function(trigger, actor)
    ...
    characters_inside = math.max(characters_inside - 1, 0)

    -- Rotates the door back to original position smoothly, once nobody is inside
    if (characters_inside == 0) then
