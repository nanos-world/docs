...
Please refer to [Unreal's Official tutorial - Working with Levels](https://dev.epicgames.com/documentation/en-us/unreal-engine/managing-multiple-levels-in-unreal-engine).
...
## Rules for creating a Map/Level {/* #rules-for-creating-a-maplevel */}
...
Now let's create the Map itself, in Unreal maps are called [Levels](https://dev.epicgames.com/documentation/en-us/unreal-engine/levels-in-unreal-engine), to do so:
...
After that, with your Plane \(floor\) selected, let's just scale it to `X = 10, Y = 10, Z = 10` and centralize it on `X = 0, Y = 0, Z = 0`:
...
Great job, now we can see the stuff! Now let's add some color to it, applying color and textures in Unreal are made through [Materials](https://dev.epicgames.com/documentation/en-us/unreal-engine/unreal-engine-materials), for that, let's just create a Material
...
## Adding a Thumbnail {/* #adding-a-thumbnail */}
...
## Configuring your Map {/* #configuring-your-map */}

Also it is possible to add a Configuration File, Custom Data and even an exclusive Package (scripting) for your map! Please refer to [Map Script and Configuration](/core-concepts/packages/packages-guide.mdx#map) for more information!
