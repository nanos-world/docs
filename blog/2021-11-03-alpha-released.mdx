...
Also it received new methods: [:SetFreeze()](/docs/scripting-reference/classes/webui#function-setfreeze), [:Resize()](/docs/scripting-reference/classes/scenecapture#function-resize) and [:SetAutoResize()](/docs/scripting-reference/classes/webui).
...
* Added new method for all actors: [:SetLifeSpan(time)](/docs/scripting-reference/classes/base-classes/actor#function-setlifespan) which will automatically destroy this actor after `time` seconds.
* Added new method for all actors: [:IsBeingDestroyed()](/docs/scripting-reference/classes/base-classes/entity#function-isbeingdestroyed) which will return if this actor is being destroyed (useful to check from inside other Events such as `Drop` or `Ungrab`).
* Added new parameter for all actors [:AttachTo()](/docs/scripting-reference/classes/base-classes/actor#function-attachto) method: `lifespan_when_detached`, which will destroy the actor after `time` seconds after it's detached.
* Now event [TakeDamage](/docs/scripting-reference/classes/base-classes/actor) has a new argument: `causer` (the object/actor which caused the damage). Also added new damage types: `RunOverVehicle` and `RunOverProp`.
* Now event [Hit](/docs/scripting-reference/classes/base-classes/actor) has new arguments: `impact_force`, `normal_impulse`, `impact_location` and `velocity`.
* Now [:SetLocation()](/docs/scripting-reference/classes/base-classes/actor#function-setlocation) will try to find the better spot to teleport the Actor (to avoid collisions).
...
* Added [:SetDamageMultiplier()](/docs/scripting-reference/classes/base-classes/damageable#function-setdamagemultiplier) change the damage multiplier depending on the bone damaged.
* Added [:SetHiddenBone()](/docs/scripting-reference/classes/character) to make invisible any part of the body.
* Added [:SetJumpZVelocity()](/docs/scripting-reference/classes/base-classes/pawn#function-setjumpzvelocity) to make the character jump higher.
* Improved [:PlayAnimation()](/docs/scripting-reference/classes/character#function-playanimation) got new parameters (blend times, loop and play rate), and now it supports loading any type of animation (Montage or Sequence).
* Removed `:SetInitialLocation()`, it has been replaced with [:Respawn()](/docs/scripting-reference/classes/base-classes/damageable#function-respawn).

**Character Events additions/improvements:**
* Added [AttemptEnterVehicle](/docs/scripting-reference/classes/character) called when Character tries to enter a vehicle, returns false to prevent it.
* Added [AttemptLeaveVehicle](/docs/scripting-reference/classes/character) called when Character tries to leave a vehicle, returns false to prevent it.
* Added [AttemptReload](/docs/scripting-reference/classes/character) called when Character tries to reload it's weapon, returns false to prevent it.
* Added [Interact](/docs/scripting-reference/classes/character) called when Characters tries to Pick Up a Pickable or Grab a Prop.
* Added [Highlight](/docs/scripting-reference/classes/character) called when Characters looks at a Prop or Pickable being able to pick up it.
...
* Added several new Getters from it's properties.
* Added [:SetRecoil()](/docs/scripting-reference/classes/weapon#function-setrecoil) how much the weapon recoils physically.
* Added [:SetWallbangSettings()](/docs/scripting-reference/classes/weapon#function-setwallbangsettings) configure if bullets wallbangs walls.
* Added [:SetAnimationFire()](/docs/scripting-reference/classes/weapon#function-setanimationfire) animation on Weapon Skeletal when firing.
* Added [:SetSoundFireLastBullets()](/docs/scripting-reference/classes/weapon#function-setsoundfirelastbullets) sound when `N` last bullets are remaining in the clip.
* Improved [:SetAnimationReload()](/docs/scripting-reference/classes/weapon#function-setanimationreload) now has `play_rate` parameter.
...
* Added [:GetDuration()](/docs/scripting-reference/classes/sound#function-getduration).
* Improved [:FadeIn()](/docs/scripting-reference/classes/sound#function-fadein) and [:FadeOut()](/docs/scripting-reference/classes/sound#function-fadeout) parameters.
...
* Added new methods [Server.GetVersion()](/docs/scripting-reference/static-classes/server#static-function-getversion), [Server.GetIP()](/docs/scripting-reference/static-classes/server#static-function-getip), [Server.GetPort()](/docs/scripting-reference/static-classes/server#static-function-getport), [Server.GetQueryPort()](/docs/scripting-reference/static-classes/server#static-function-getqueryport) and several missing getters.
...
* New parameters have been added to [Client.SetCameraLocation()](/docs/scripting-reference/classes/player#function-setcameralocation) and [Client.SetCameraRotation()](/docs/scripting-reference/classes/player#function-setcamerarotation) to enable smooth movements with camera in a certain time.
* New methods for drawing debug shapes in 3D world: [Client.DrawDebug...()](/docs/scripting-reference/static-classes/client).
* Changed [Client.SetHighlight()](/docs/scripting-reference/static-classes/client) parameters, now it takes a Enum to define how the highlight will be.
* Added new method [Client.SetSteamRichPresence()](/docs/scripting-reference/static-classes/client) to customize the message in Steam Friends list.
...
* Added new method: [Timer.Bind(timer_id, actor)](/docs/scripting-reference/static-classes/timer#static-function-bind) - this will associate a timer to an actor, when the actor is destroyed, this timer is destroyed too and will never be called.
* Added new method: [Timer.IsValid(timer_id)](/docs/scripting-reference/static-classes/timer#static-function-isvalid).
...
* Added new Searchers to [Package.Require()](/docs/scripting-reference/static-classes/package#static-function-require), we now currently support 5 searchers, which are looked in the following order:
    1. Relative to `current-file-path/`
...
* [Package.Require()](/docs/scripting-reference/static-classes/package#static-function-require) now can be called in runtime and at any moment.
* [Package.RequirePackage()](/docs/scripting-reference/static-classes/package) now will load a package as a library inside the current package, it means the package loaded will now be automatically sent to clients, and all paths it loads is relative to the package itself (and not to the one which is requiring it anymore).
* Verbose logs will now display the current thread ID.


## Renamed Default Packages {/* #renamed-default-packages */}
