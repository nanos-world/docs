...
This would cause a breakage change, but with the addition of the **Package Compatibility**, the outdated packages will continue to work.
...
Some classes will also be renamed for better standardization. For example **Character** and **Vehicle**, they would become `CharacterMannequin` and `VehicleWheled`, which allows new classes of other types e.g. `VehicleWater` or `CharacterGeneric` for example.
