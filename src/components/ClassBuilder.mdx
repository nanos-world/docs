...
import { FunctionToolTip, StaticFunctionToolTip, InlineFunctionToolTip, EventToolTip } from '@site/src/components/Tooltips.jsx';
...
export const AuthorityLabels = {
	client: <>This class can only be spawned on 🟧 <a href="/docs/next/core-concepts/scripting/authority-concepts">Client</a> side.</>,
	server: <>This class can only be spawned on 🟦 <a href="/docs/next/core-concepts/scripting/authority-concepts">Server</a> side.</>,
	both: <>This class can be spawned on both 🟧 <a href="/docs/next/core-concepts/scripting/authority-concepts">Client</a> and 🟦 <a href="/docs/next/core-concepts/scripting/authority-concepts">Server</a> side (if you spawn it on client, it won't be synchronized with other players).</>,
}

<!-- Authority Label Map for Static Classes -->
export const AuthorityLabelsStatic = {
	client: <>This static class can be accessed only on 🟧 <a href="/docs/next/core-concepts/scripting/authority-concepts">Client</a> side.</>,
	server: <>This static class can be accessed only on 🟦 <a href="/docs/next/core-concepts/scripting/authority-concepts">Server</a> side.</>,
	both: <>This static class can be accessed on both 🟧 <a href="/docs/next/core-concepts/scripting/authority-concepts">Client</a> and 🟦 <a href="/docs/next/core-concepts/scripting/authority-concepts">Server</a> side.</>,
}
...
export function GetGenericExample(example, context) {
	return <CodeBlock className="language-lua api-examples" title={ `${context ? `(${context}) ` : ""}${example.title}` }>
		{ example.content }
...
export function GetEventExample(class_name, event_data, example, context) {
	return GetGenericExample({ content: GetEventSignature(class_name, event_data, `\t${example.content.replaceAll('\n', "\n\t")}`), title: example.title }, context);
}
...
export const FunctionDeclaration = ({ function_data, is_static, class_name }) => (
	<>
		...
		{Array.isArray(function_data.examples) && function_data.examples.length > 0 ?
			<details>
				<summary>{ is_static ? `${class_name}.${function_data.name}` : `my_${class_name.toLowerCase()}:${function_data.name}` } Examples</summary>
				{function_data.examples.map(function(example, index) {
					return <>
						{ GetGenericExample(example) }
					</>
				})}
			</details>
		: <></>}
...
export const InlineFunctionNameDeclaration = ({ function_parameters }) => (
	<Tippy interactive={true} maxWidth={1200} animation={"scale-subtle"} content={<InlineFunctionToolTip function_parameters={function_parameters} />}>
		<Link className={"hover-link"}>with this format</Link>
	</Tippy>
);
...
export const ExamplesDeclaration = ({ type, name }) => {
	...
	return <>
		...
		{ class_data.functions && class_data.functions.map((value) =>
			value.examples && value.examples.map((example) => GetGenericExample(example, `my_${ name.toLowerCase() }:${ value.name }`))
		)}
