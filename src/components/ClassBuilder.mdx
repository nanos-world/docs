...
export function GetElementsByTypes(types) {
	return types.split(",").map((type) => GetElementByType(type)).reduce((prev, next) => [prev, <span className="comma-separator"></span>, next])
}
...
export function GetReturnPart(function_data) {
	if (!function_data.return)
		return "";
	const commas = function_data.return.type.split(",");
	if (commas.length == 1)
		return "local ret = ";
	return `local${commas.map((value, i) => " ret_0" + (i + 1))} = `;
}

<!-- Generates Function Example Codeblock -->
export function GetFunctionExample(function_data) {
	return `${GetReturnPart(function_data)}my_entity:${function_data.name}(${GetParametersList(function_data.parameters)})`;
}
	...
	<>
		...
		<blockquote>
			...
			{ !function_data.return ? "" :
				<>
					<br/><br/><>Returns </>
					{ GetElementsByTypes(function_data.return.type) }
			...
			<table style={{ width: "100%", display: "table" }}>
				...
				<tbody>
					{function_data.parameters.map(function(value) {
						return <tr>
							<td>{ GetElementsByTypes(value.type) }</td>
...
export const EventDeclaration = ({ event_data, class_name }) => (
	<>
		...
		<blockquote dangerouslySetInnerHTML={{ __html: `${ event_data.description_long ? event_data.description_long : event_data.description }${ event_data.return ? "<br/><br/>" + event_data.return.description : ""}` }}></blockquote>
		<CodeBlock className="language-lua">
...
export const FunctionNameDeclaration = ({ function_data }) => (
	<Tippy interactive={true} maxWidth={1200} animation={"scale-subtle"} content={<FunctionToolTip function_data={function_data} />}>
		<b>
			<Link to={`#${function_data.name.toLowerCase()}`} className={"hover-link"}>
...
export const EventNameDeclaration = ({ class_name, event_data }) => (
	<Tippy interactive={true} maxWidth={1200} animation={"scale-subtle"} content={<EventToolTip class_name={class_name} event_data={event_data} />}>
		<b>
			<Link to={`#${event_data.name.toLowerCase()}`} className={"hover-link"}>
		...
		<tbody>
			{functions_list.map(function(value) {
				return <tr>
					...
					<td>{ value.return ? GetElementsByTypes(value.return.type) : "" }</td>
					<td><FunctionNameDeclaration function_data={ value } /></td>
					<td style={{"word-break": "break-word"}} dangerouslySetInnerHTML={{ __html: value.description }}></td>
				</tr>;
			})}
		</tbody>
	...
	<table style={{ width: "100%", display: "table" }}>
		...
		<tbody>
			{class_data.events.map(function(value) {
				return <tr>
					<td width="50px">{ GetAuthorityType(value.authority) } </td>
					<td><EventNameDeclaration class_name={ class_data.name } event_data={ value } /></td>
					<td style={{"word-break": "break-word"}} dangerouslySetInnerHTML={{ __html: value.description }}></td>
				</tr>;
			})}
		</tbody>
