...
export function GetReturnDescriptionList(return_list) {
	const return_list_plain = return_list.map((return_data) => return_data.description).reduce((prev, next) => [prev, ", ", next]);
...
export const HeaderDeclaration = ({ class_data, image, is_static, is_open_sourced }) => (
	<>
		<p dangerouslySetInnerHTML={{ __html: class_data.description }}></p>
		{ image ? <p><img src={image} /></p> : "" }
		{ is_open_sourced ? <OpenSourceAdmonition /> : "" }
