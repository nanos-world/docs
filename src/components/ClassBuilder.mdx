...
export const TypeToAPIFolder = { StandardLibrary: "StandardLibraries", Struct: "Structs", Class: "Classes", StaticClass: "StaticClasses", UtilityClass: "UtilityClasses", UtilityClasses: "UtilityClass",  };

<!-- API Source URL -->
export const APISourceURL = ({ type, class_name }) => (
	<Admonition type="note" icon="🧑‍💻" title="API Source">
		The methods and events descriptions from this page are defined in our <a href={`https://github.com/nanos-world/api/blob/main/${TypeToAPIFolder[type]}/${class_name}.json`}>API Repository on GitHub</a>!
	</Admonition>
);
