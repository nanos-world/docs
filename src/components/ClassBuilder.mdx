...
export const FunctionNameDeclaration = ({ function_data }) => (
	<Tippy interactive={true} maxWidth={1000} animation={"scale-subtle"} content={<FunctionToolTip function_data={function_data} />}>
		<b>
			<Link to={`#${function_data.name.toLowerCase()}`}>
...
export const EventNameDeclaration = ({ class_name, event_data }) => (
	<Tippy interactive={true} maxWidth={1000} animation={"scale-subtle"} content={<EventToolTip class_name={class_name} event_data={event_data} />}>
		<b>
			<Link to={`#${event_data.name.toLowerCase()}`}>
		...
		<tbody>
			{functions_list.map(function(value) {
				return <tr>
					...
					<td style={{ "white-space": "nowrap" }}>{ value.return ? GetElementsByTypes(value.return.type) : "" }</td>
