...
## Downloading Assets Development Kit {/* #downloading-assets-development-kit */}
...
You will find a file `NanosWorldADK.uproject` in there, just open it with Unreal Engine (make sure you have [Set Up Unreal Engine](setting-up-ue.mdx) before proceeding).

The first time opening it may take some time as the initial shaders must be compiled.


After it finishes compiling all shaders, you can proceed to the [next step to import an asset](importing-assets.mdx) or take a look below in all available tools.
...
## Updating to a newer ADK version {/* #updating-to-a-newer-adk-version */}
...
## Tools available in the ADK project {/* #tools-available-in-the-adk-project */}
...
### Placeholder Blueprints {/* #placeholder-blueprints */}
#### `NanosWorld/Blueprints/Placeholders/` {/* #nanosworldblueprintsplaceholders */}
...
#### `NanosWorld/Blueprints/Utility/WBP_LuaCodeGenerator` {/* #nanosworldblueprintsutilitywbp_luacodegenerator */}
...
### Assets.toml Generator {/* #assetstoml-generator */}
#### `NanosWorld/Blueprints/Utility/WBP_AssetsTomlGenerator` {/* #nanosworldblueprintsutilitywbp_assetstomlgenerator */}
...
### Sun & Sky Actor {/* #sun--sky-actor */}
#### `NanosWorld/Blueprints/World/BP_SunSky` {/* #nanosworldblueprintsworldbp_sunsky */}

This is a pre-configured **Sun & Sky Blueprint** which can be used and easily replaced by nanos world Official Sky with `Sky.Spawn()`. This is useful to get an approximate result of how the light will look like in-game if you decide to Spawn the Default Sun.

:::info

**Note:** Overriding the Sun through Scripting with `Sky.Spawn()`, will respawn the Sun actor, which means no configuration did on the Sun & Sky actor will persist.
...
#### `NanosWorld/MaterialLibrary/PhysicalMaterials/` {/* #nanosworldmateriallibraryphysicalmaterials */}
...
### Mannequin Skeleton {/* #mannequin-skeleton */}
#### `NanosWorld/Characters/Common/SKEL_Mannequin` {/* #nanosworldcharacterscommonskel_mannequin */}
...
#### `NanosWorld/Blueprints/Utility/ThumbnailGenerator` {/* #nanosworldblueprintsutilitythumbnailgenerator */}
