...
### Adding Items Automatically {/* #adding-items-automatically */}
...
```lua showLineNumbers title="Shared/Index.lua"
-- Inherits from Weapon defining custom values which will be read from Spawn Menu
-- The class must exist on both sides (so Shared/) to be spawned, the custom values are only used on Client side
...
You can check [default-weapons](https://github.com/nanos-world/default-weapons/blob/master/Shared/Index.lua), [default-vehicles](https://github.com/nanos-world/default-vehicles/blob/master/Shared/Index.lua) or [ts-fireworks-tools](https://github.com/gtnardy/nanos-world-ts-fireworks) as examples!
...
You must call `Sandbox.SpawnMenu.AddItem` from both client and server side. Each side has its own parameters.
...
function SpawnMyIncredibleEntity(location, rotation, tab, id)
    local my_stuff = Prop(location, rotation, "nanos-world::SM_Crate_07")
...
All Classes inherited from <Classes.Character /> and <Classes.CharacterSimple /> will appear in the npcs tab by default.
...
- Tab **weapons**: `rifles`, `smgs`, `pistols`, `shotguns`, `sniper-rifles`, `special`, `grenades`, `melees`.
- Tab **vehicles**: `wheeled`, `water`.
- Tab **tools**: `tool-guns`, `spawners`, `constrainers`.
- Tab **npcs**: `humanoids`, `robots`.
