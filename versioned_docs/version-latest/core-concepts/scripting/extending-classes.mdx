...
## Extending Native Classes {/* #extending-native-classes */}
...
`self:Super(...)` calls the original method with the same name, and can only be used inside the overriding method. This is different from [Inherited Classes](/core-concepts/scripting/inheriting-classes.mdx), where you call the parent methods with `self.Super:MethodName(...)`.
