...
### Communicating with Packages in the same Side {/* #communicating-with-packages-in-the-same-side */}
...
Events.Subscribe("MyAwesomeEvent", function(parameter1, parameter2, parameter3)
    ...
    Console.Log("Received " .. parameter3.important_stuff) -- Received omg!
end)
```


### Sending data through the network {/* #sending-data-through-the-network */}
...
## Exporting Variables Globally {/* #exporting-variables-globally */}

Another way of communicating is using <MethodReference type="StaticClass" class_name="Package" method_name="Export" show_class_name /> method, it allows exposing variables (tables, functions, etc) globally so all other Packages can access it directly.
...
function SpawnCoolWeapon(location, rotation)
    local cool_weapon = Weapon(location or Vector(), rotation or Rotator(), "nanos-world::SK_AK47")
    return cool_weapon
end
...
### Calling an Exported Function from another Package {/* #calling-an-exported-function-from-another-package */}

Now you can access that directly from other packages:

:::caution

An exported variable only exists after the Package which exports it has loaded. If your Package depends on it, add the exporting Package to the `packages_requirements` of your `Package.toml`, so it's always loaded before yours. See [Package Loading & Lua Environment](/core-concepts/packages/package-loading-and-lua-environment.mdx).
