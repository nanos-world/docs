...
### Installing SteamCMD on Linux {/* #installing-steamcmd-on-linux */}
...
Install SteamCMD using cURL (this will download and extract steamcmd in the current folder):
```shell
...
## Downloading with SteamCMD {/* #downloading-with-steamcmd */}
...
#### 1. After downloaded run `steamcmd` executable, to enter into it's console: {/* #1-after-downloaded-run-steamcmd-executable-to-enter-into-its-console */}

```steamcmd
 
```

#### 2. You can optionally define an installation folder to install nanos world server: {/* #2-you-can-optionally-define-an-installation-folder-to-install-nanos-world-server */}
...
#### 3. The next step is to login on SteamCMD, you can login anonymously with: {/* #3-the-next-step-is-to-login-on-steamcmd-you-can-login-anonymously-with */}

```steamcmd
login anonymous
```

#### 4. You can then install or update nanos world server using it's app id: `1936830`: {/* #4-you-can-then-install-or-update-nanos-world-server-using-its-app-id-1936830 */}
...
By default the server runs as the **Playtest** Steam App, which is what players currently use to join. You can change it with the `--steam_app` parameter (`playtest` or `game`), players can only join servers running the same Steam App they are playing on. See [Server Configuration](/core-concepts/server-manual/server-configuration.mdx#command-line-parameters).

:::

#### And you are done! You can quit the console: {/* #and-you-are-done-you-can-quit-the-console */}
