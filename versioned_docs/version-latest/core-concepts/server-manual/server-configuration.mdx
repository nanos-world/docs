...
| `max_file_transfer_rate <1-max_send_rate>` | Changes the max file transfer rate per client |
...
| `--max_file_transfer_rate` | 1-64 | Sets the server max file transfer rate per client in MB/s - this must be smaller than max_send_rate |
