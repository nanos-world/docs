...
| **`map`** | `<map_package>` | Changes the map to another map Package, reloading all Packages and reconnecting the Players |
...
| **`--ip`** | string | Address the server binds to |
| **`--steam_app`** | `game` or `playtest` | Which Steam App the server runs as. Players can only join servers running the same Steam App they are playing on<br/><br/>Default is <code>playtest</code> |
| **`--map`** | string | Map Package to load |
| **`--port`** | 1024-65535 | Server port |
| **`--query_port`** | 1024-65535 | Server Query port (must be different than server port) |
| **`--announce`** | 0 or 1 | Whether to announce the server in the Server List |
...
| **`--max_file_transfer_rate`** | 128-max_send_rate | Sets the server max file transfer rate per client in KB/s<br/><br/>Values above <code>max_send_rate</code> are clamped down to it |
...
**Flag** Value Types don't require any argument, just pass the parameter like `--profiling`.

:::

:::info

Two special parameters must be passed as the **first** argument:

* **`--cli`** runs the [Command Line Interface](/core-concepts/server-manual/command-line-interface.mdx) instead of starting the server, e.g. `./NanosWorldServer.exe --cli install package sandbox`.
* **`--tracy`** (Linux only, through `NanosWorldServer.sh`) starts the server build with the [Tracy Profiler](/core-concepts/scripting/profiling.mdx) integration. On Windows, run `NanosWorldServerTracy.exe` instead.
...
```
```shell
./NanosWorldServer.exe --name "nanos world Amazing Sandbox" --description "Awesome Sandbox Server" --map "default-testing-map" --game_mode "sandbox" --packages "battlefield-kill-ui,ts-fireworks-tools" --port 7777 --query_port 7778 --max_players 32 --logo "https://i.imgur.com/vnB8CB5.jpg"
```

Or, just start the server passing with all configs set and auto downloads the packages and assets if needed:

```shell
./NanosWorldServer.exe --name "nanos world Amazing Sandbox" --description "Awesome Sandbox Server" --map "default-testing-map" --game_mode "sandbox" --packages "battlefield-kill-ui,ts-fireworks-tools" --port 7777 --query_port 7778 --max_players 32 --auto_download --logo "https://i.imgur.com/vnB8CB5.jpg"
```
...
```
```shell
./NanosWorldServer.sh --name "nanos world Amazing Sandbox" --description "Awesome Sandbox Server" --map "default-testing-map" --game_mode "sandbox" --packages "battlefield-kill-ui,ts-fireworks-tools" --port 7777 --query_port 7778 --max_players 32 --logo "https://i.imgur.com/vnB8CB5.jpg"
```

Or, just start the server passing with all configs set and auto downloads the packages and assets if needed:

```shell
./NanosWorldServer.sh --name "nanos world Amazing Sandbox" --description "Awesome Sandbox Server" --map "default-testing-map" --game_mode "sandbox" --packages "battlefield-kill-ui,ts-fireworks-tools" --port 7777 --query_port 7778 --max_players 32 --auto_download --logo "https://i.imgur.com/vnB8CB5.jpg"
```
...
By default the server runs at 30 ticks per second (or the `max_tick_rate` configured in the Config.toml), the server runs in an infinite loop in that frequency, and inside that loop all server operations are executed, such as receiving and sending network packets, triggering lua events, executing functions or callbacks, and so on.

At 30 ticks per second, each tick has about 33 milliseconds to do all its work. If a single tick takes longer than that, this warning will show up.
