...
| `--distance_optimization` | 0-9 | Sets the distance optimization level to improve network usage by reducing sync from distant actors - 0 disables it, 4 is a good middle term, 9 is the most aggressive |
| `--thread_pool_count` | number | Sets the number of threads initialized to execute async operations (e.g. from HTTP, Database, File) - default is 8 (or the number of CPU cores) |
