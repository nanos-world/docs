...
| **`max_sync_rate`** | Max actors sync rate in Hz. How much actors location/rotation/velocity will synchronize per second, limited by `max_tick_rate` |
| **`max_send_rate`** | Server network send rate per client in KB/s. Limits bandwidth for each connected player. Recommended setting to the lowest needed value as higher rates may negatively impact the server performance when having players with bad connections |
| **`max_file_transfer_rate`** | Changes the max file transfer rate per client |
| **`distance_optimization`** | Sets the distance optimization level to improve network usage by reducing sync from distant actors |
| **`log_level`** | Which Log level to output |
| **`game_mode`** | [game-mode](/core-concepts/packages/packages-guide.mdx#game-mode) package to load -  you can only load one game-mode at once. |
| **`loading_screen`** | [loading-screen](/core-concepts/packages/loading-screen.mdx) package to load - you can only load one loading-screen at once. |
| **`packages`** | [script](/core-concepts/packages/packages-guide.mdx#script) packages to load |
| **`map`** | [map](/core-concepts/packages/packages-guide.mdx#map) package to load |
...
It is possible to define custom commands as well, for that please refer to [Console.RegisterCommand()](/scripting-reference/static-classes/console.mdx#static-function-registercommand).
...
| **`--compression`** | 0-9 | Sets the compression level to use in some networking operations<br/><br/><code>0</code> disables it, <code>1</code> is the fastest and <code>9</code> is the slower but highest compression ratio |
...
| **`--distance_optimization`** | 0-9 | Sets the distance optimization level to improve network usage by reducing sync from distant actors<br/><br/><code>0</code> disables it, <code>4</code> is a good middle term, <code>9</code> is the most aggressive |
