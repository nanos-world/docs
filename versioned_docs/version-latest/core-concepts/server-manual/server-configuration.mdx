...
| **`--ip`** | string | Server IP |
| **`--steam_app`** | `game` or `playtest` | Which Steam AppID the server runs as. Players can only join servers with the same AppID they are playing on<br/><br/>Default is <code>playtest</code> |
