...
2. Use SteamCMD to [download nanos world™ Dedicated Server](/core-concepts/server-manual/server-installation).
...
To create a new Package, lets use the handy [CLI](/core-concepts/server-manual/command-line-interface) tool to speed up the process. Open a terminal in your server folder and run the following command:
