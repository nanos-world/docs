...
During first-time startup, you will be prompted with several questions:
...
After that, you will end up with a new folder inside `Packages/` called `my-awesome-package/`. Inside that there will be 3 other folders: `Server/`, `Client/` and `Shared/`, and a package file `Package.toml`.
 ...
 Additionally, it will also create a `Config.toml` file at the root of the NanosWorld folder (the same folder as your `NanosWorld.exe` or `NanosWorld.sh` executables).
