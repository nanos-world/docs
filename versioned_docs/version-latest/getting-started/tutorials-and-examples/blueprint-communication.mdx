...
The server doesn't run Unreal, so most Blueprint interaction happens on the **Client Side**: binding Event Dispatchers and getting or setting Blueprint properties are client-only.

Calling a Blueprint Event with `CallBlueprintEvent` works on both sides: when called on the server, the call is sent to all clients.
...
### Calling a Custom Event from Lua {/* #calling-a-custom-event-from-lua */}
...
## Blueprint Event Dispatcher {/* #blueprint-event-dispatcher */}
...
### Creating an Event Dispatchers {/* #creating-an-event-dispatchers */}
...
### Calling a Dispatcher from Blueprint {/* #calling-a-dispatcher-from-blueprint */}
...
### Binding Dispatchers from Lua {/* #binding-dispatchers-from-lua */}
