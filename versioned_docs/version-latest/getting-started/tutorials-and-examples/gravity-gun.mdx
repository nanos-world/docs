...
Events.SubscribeRemote("GravityGun:Grab", function(player, object, is_grabbing)
...
Events.SubscribeRemote("GravityGun:Move", function(player, object, location)
...
Learn more about validating remote events in the [Never Trust the Client](/core-concepts/scripting/security-remote-events.mdx) guide.
...
Input.Subscribe("MouseUp", function(key_name)
	...
	if (key_name == "LeftMouseButton") then
		...
		if (picking_object) then
			-- Calls server to re-enable gravity (if possible) and update its last position
			Events.CallRemote("GravityGun:Grab", Reliability.Reliable, picking_object, false)
		...
		if (trace_result.Success and trace_result.Entity) then
			...
			Events.CallRemote("GravityGun:Grab", Reliability.Reliable, picking_object, true)
...
-- When the mouse wheel scrolls, updates the Distance of the object from the camera
Input.Subscribe("MouseScroll", function(mouse_x, mouse_y, delta)
	...
	distance = math.max(distance + delta * 25, 100)
end)
...
Client.Subscribe("Tick", function(delta_time)
	...
	-- Calls remote to update its location. We use Unreliable as this is sent every frame, if a packet is lost, the next one will correct it anyway
	Events.CallRemote("GravityGun:Move", Reliability.Unreliable, picking_object, end_location)
end)
```
